...
public class NativeLoader {
	...
	public static class NativeLibraryLoadError extends UnsatisfiedLinkError {
		...
		public NativeLibraryLoadError(String message, Throwable cause) {
			// The convenience super-constructor, with cause as second argument, is only
			// available from
			// Java 1.7
			super(message);
			initCause(cause);
	...
	/**
	 * Load the library named, if osNameCheck is the current operating system and
	 * osArchCheck is the current architecture.
	 * 
	...
	private static void loadLibraryFromJar(String library) {
		...
		InputStream in = classLoader.getResourceAsStream("natives/" + osName + "/" + osArch + "/" + library);
		if (in == null) {
			in = classLoader.getResourceAsStream("natives/" + osName + "/" + library);
		}
		if (in == null) {
			throw new NativeLibraryLoadError(
					"Couldn't find " + library + " for this platform " + osName + "/" + osArch);
		}
		try {
			// File tempDir=new File(".");
			// File tempDir=null;
			//File fileOut = File.createTempFile(library.replaceFirst("\\.[^.]*$", ""),
			//		library.replaceFirst("^.*\\.", "."),tempDir);
			File fileOut=new File(library);
